...
class TestInMemoryStore(TestCase):
    def setUp(self):
        ...
        self.test_last_op = LastOperation(
            state='test_state',
            description='testing'
        )

        self.test_svc_instance = ServiceInstance(
            service_type=self.test_service,
            context={'id': 'test_svc_inst'}
        )

    def tearDown(self):
        super().tearDown()
        # TODO this has no consistent-check.
        # A manifest has a plan and service ID but those are deleted while this one is still dependent on them
    ...
    def test_delete(self):
        ...
        # TODO a manifest is dependent on a plan and a service, it can't just be deleted if the service is non-existent
        self.store.add_manifest(self.test_manifest)
        self.store.delete_manifest(self.test_manifest.id)
        self.assertGreaterEqual(len(self.store.get_manifest()), 0)

        # TODO why two checks for manifest code?
        self.store.add_manifest(self.test_manifest)
        self.store.delete_manifest(self.test_manifest.id)
        self.assertGreaterEqual(len(self.store.get_manifest()), 0)

        # TODO unconsistent dependencies unmet
        self.store.add_service_instance(self.test_svc_instance)
